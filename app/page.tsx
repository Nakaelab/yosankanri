    ...
    return (
        <div className="animate-fade-in">
            ...
            <div className="p-4 md:p-6 space-y-6">
                {/* ===== 全体サマリー (Hero Status) ===== */}
                <div className="relative bg-slate-900 rounded-3xl p-6 md:p-8 shadow-xl overflow-hidden mb-2">
                    {/* Background decorations */}
                    <div className="absolute top-0 right-0 -mr-16 -mt-16 w-64 h-64 rounded-full bg-brand-500 opacity-20 blur-3xl pointer-events-none"></div>
                    <div className="absolute bottom-0 left-0 -ml-16 -mb-16 w-48 h-48 rounded-full bg-emerald-500 opacity-20 blur-3xl pointer-events-none"></div>

                    <div className="relative z-10 grid grid-cols-1 sm:grid-cols-3 gap-6 sm:gap-8 divide-y sm:divide-y-0 sm:divide-x divide-slate-700/50">
                        {/* 配分総額 */}
                        <div className="pt-2 sm:pt-0 pb-2 sm:pb-0 sm:pl-0">
                            <div className="flex items-center gap-2 mb-2 sm:mb-4">
                                <div className="p-2 rounded-xl bg-slate-800/80 text-brand-400 shadow-sm border border-slate-700/50">
                                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18.75a60.07 60.07 0 0 1 15.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 0 1 3 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 0 0-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 0 1-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 0 0 3 15h-.75M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm3 0h.008v.008H18V10.5Zm-12 0h.008v.008H6V10.5Z" />
                                    </svg>
                                </div>
                                <span className="text-sm font-bold text-slate-400 tracking-wider">配分総額</span>
                            </div>
                            <div className="text-3xl lg:text-4xl xl:text-5xl font-black text-white tabular-nums tracking-tight pl-1">
                                {fmtYen(totalAllocated)}
                            </div>
                        </div>
                        ...
                        <div className="pt-6 sm:pt-0 sm:pl-8">
                            <div className="flex items-center gap-2 mb-2 sm:mb-4">
                                <div className="p-2 rounded-xl bg-slate-800/80 text-amber-400 shadow-sm border border-slate-700/50">
                                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                                    </svg>
                                </div>
                                <span className="text-sm font-bold text-slate-400 tracking-wider">執行総額</span>
                            </div>
                            <div className="text-3xl lg:text-4xl xl:text-5xl font-black text-amber-400 tabular-nums tracking-tight pl-1">
                                {fmtYen(totalSpent)}
                            </div>
                        </div>
                        ...
                        <div className="pt-6 sm:pt-0 sm:pl-8">
                            <div className="flex items-center gap-2 mb-2 sm:mb-4">
                                <div className={`p-2 rounded-xl bg-slate-800/80 shadow-sm border border-slate-700/50 ${totalRemaining < 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
                                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M21 11.25v8.25a1.5 1.5 0 0 1-1.5 1.5H5.25a1.5 1.5 0 0 1-1.5-1.5v-8.25M12 4.875A2.625 2.625 0 1 0 9.375 7.5H12m0-2.625V7.5m0-2.625A2.625 2.625 0 1 1 14.625 7.5H12m0 0V21m-8.625-9.75h18c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125h-18c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125Z" />
                                    </svg>
                                </div>
                                <span className="text-sm font-bold text-slate-400 tracking-wider">残額</span>
                            </div>
                            <div className={`text-3xl lg:text-4xl xl:text-5xl font-black tabular-nums tracking-tight pl-1 ${totalRemaining < 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
